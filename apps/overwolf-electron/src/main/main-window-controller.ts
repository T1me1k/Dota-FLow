...
type PersistedWindowState = MainWindowState & {
...
export function compactBoundsFor(expandedBounds: Rectangle, workArea: Rectangle): Rectangle {
  return fitBounds({
    x: expandedBounds.x + expandedBounds.width - COMPACT_WIDTH,
    y: expandedBounds.y,
    width: COMPACT_WIDTH,
...
export class MainWindowController {
  ...
  async setCompact(compact: boolean): Promise<MainWindowState> {
    ...
    if (compact) {
      this.#expandedBounds = this.#window.isMaximized()
  ...
  async #applyInitialState(): Promise<void> {
    ...
    this.#window.setMinimumSize(FULL_MIN_WIDTH, FULL_MIN_HEIGHT);
    this.#window.setResizable(true);
    this.#window.setBounds(this.#expandedTarget());
  }
  ...
  async #animateTo(target: Rectangle): Promise<void> {
    ...
    await new Promise<void>((resolve) => {
      const timer = setInterval(() => {
        if (animationId !== this.#animationId || this.#window.isDestroyed()) {
          clearInterval(timer);
          resolve();
          return;
        }
        ...
        this.#window.setBounds({
          x: interpolate(start.x, target.x),
          y: interpolate(start.y, target.y),
  ...
  async #persist(): Promise<void> {
    ...
    await writeFile(settingsPath(), `${JSON.stringify({
      ...this.#state,
      expandedBounds: this.#expandedBounds
    }, null, 2)}\n`, 'utf8');
  }
}
