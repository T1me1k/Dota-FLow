...
const INVOKE_CHANNELS = new Set([
  'runtime:get-status','runtime:get-snapshot','runtime:start','runtime:stop',
  'capture:start','capture:stop','capture:get-status','capture:open-folder',
  'manual-context:send','coach-timer:start','diagnostics:get','diagnostics:export',
  'window:get-state','window:set-compact','window:set-always-on-top','window:close'
]);
function delay(ms: number): Promise<void> { return new Promise((resolve) => setTimeout(resolve, ms)); }
function isHandlerRegistrationRace(error: unknown): boolean { const message = error instanceof Error ? error.message : String(error); return /no handler registered|handler.*not registered|channel.*not found/i.test(message); }
async function invokeWithStartupRetry(channel: string, ...args: unknown[]): Promise<unknown> {
  ...
  for (let attempt = 1; attempt <= IPC_RETRY_ATTEMPTS; attempt += 1) {
    ...
    catch (error) { lastError = error; if (!isHandlerRegistrationRace(error) || attempt === IPC_RETRY_ATTEMPTS) throw error; await delay(IPC_RETRY_DELAY_MS); }
  }
  throw lastError;
}
function subscribe(channel: string, listener: (payload: unknown) => void): () => void { const wrapped = (_event: unknown, payload: unknown) => listener(payload); ipcRenderer.on(channel, wrapped); return () => ipcRenderer.removeListener(channel, wrapped); }
...
contextBridge.exposeInMainWorld('dotaFlow', {
  ...
  showOverlay: () => invokeWithStartupRetry('dota-flow:show-overlay'), hideOverlay: () => invokeWithStartupRetry('dota-flow:hide-overlay'),
  getCaptureStatus: () => invokeWithStartupRetry('dota-flow:get-capture-status'), startCapture: () => invokeWithStartupRetry('dota-flow:start-capture'), stopCapture: () => invokeWithStartupRetry('dota-flow:stop-capture'),
...
contextBridge.exposeInMainWorld('dotaFlowRuntime', { bridgeVersion: BRIDGE_VERSION, invoke: invokeAllowlisted, subscribe: (listener: (snapshot: unknown) => void) => subscribe('runtime:snapshot', listener) });
