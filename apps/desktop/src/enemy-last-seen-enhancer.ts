...
function renderRows(container:HTMLElement,data:LastSeenModel){
  ...
  container.replaceChildren(...rows.map(row=>{const article=document.createElement('article');article.className=`enemy-last-seen-row ${statusClass(row.status)}`;article.innerHTML='<i></i><span><b></b><small></small></span><strong></strong>';article.querySelector<HTMLElement>('i')!.textContent=heroName(row.hero).slice(0,2).toUpperCase();article.querySelector<HTMLElement>('b')!.textContent=heroName(row.hero);article.querySelector<HTMLElement>('small')!.textContent=statusText(row.status);const value=article.querySelector<HTMLElement>('strong')!;value.textContent=row.status==='MISSING'&&row.timerVisible?`${Math.max(0,Math.floor(Number(row.elapsedSec)||0))}с`:'';value.hidden=!value.textContent;return article}))
}
...
function renderPanel(panel:HTMLElement,data:LastSeenModel){const text=c();panel.querySelector<HTMLElement>('.kicker')!.textContent=text.kicker;panel.querySelector<HTMLElement>('h3')!.textContent=text.title;panel.querySelector<HTMLElement>('.enemy-last-seen-source')!.textContent=`${text.source}: ${String(data.sourceStatus??'UNAVAILABLE')}`;panel.querySelector<HTMLElement>('.enemy-last-seen-summary')!.textContent=`${text.missingCount}: ${Number(data.missingCount)||0} · ${text.quality}: ${String(data.dataQuality??'UNAVAILABLE')}`;panel.querySelector<HTMLElement>('footer')!.textContent=text.privacy;renderRows(panel.querySelector<HTMLElement>('.enemy-last-seen-rows')!,data)}
function renderLive(){
