...
export function createProfileGroup({ ITEMS, benchmark, condition }) {
  ...
  profiles.juggernaut = makeProfile({
    ...
    buildPlans: [
      makePlan(ITEMS, 'juggernaut', JUGGERNAUT_CALIBRATION, { id:'balanced', name:'Maelstrom tempo into sustained objectives', scenarioTags:['balanced'], priority:89, itemKeys:['maelstrom','manta','bkb','scepter'], reasons:['balanced_draft'], optional:['abyssal'], situational:['satanic'] }),
      makePlan(ITEMS, 'juggernaut', JUGGERNAUT_CALIBRATION, { id:'pickoff', name:'Diffusal isolated-target tempo', scenarioTags:['enemy_mobile','enemy_mana_dependent'], priority:94, itemKeys:['diffusal','manta','bkb','scepter'], reasons:['enemy_mobile','enemy_mana_dependent'], optional:['abyssal'], situational:['butterfly'] }),
    ...
    spikes: [
      makeSpike(condition, 'juggernaut', JUGGERNAUT_CALIBRATION, { id:'level_6', publicId:'jugg_level_6', name:'Omnislash level 1', priority:58, trigger:[['level_gte',6]], expectedMinute:7, earlyToleranceMin:1, lateToleranceMin:2.5, activeDurationSec:180, fadeDurationSec:120, requires:[{type:'ultimate_ready',message:'Omnislash must be ready'},{type:'min_health_pct',value:0.55,message:'Do not search for isolation while too low to survive the approach'}], permanent:{fight:6,initiation:4}, window:{fight:21,connect:12}, actions:{FIGHT:20,CONNECT:12}, recommendation:'Look for a low-unit-count target and avoid spending Omnislash into nearby creeps, summons or immediate saves.' }),
      makeSpike(condition, 'juggernaut', JUGGERNAUT_CALIBRATION, { id:'diffusal', publicId:'jugg_diffusal', name:'Diffusal tempo', priority:75, trigger:[['item_owned',getItem(ITEMS,'diffusal').id]], expectedMinute:13, earlyToleranceMin:1.8, lateToleranceMin:3, activeDurationSec:300, fadeDurationSec:180, permanent:{fight:15,mobility:9,initiation:6}, window:{fight:19,pressure:9}, actions:{FIGHT:20,CONNECT:11,PRESSURE:9}, recommendation:'Use the slow to secure a short kill or outer tower, then return to efficient farm before the response arrives.' }),
      makeSpike(condition, 'juggernaut', JUGGERNAUT_CALIBRATION, { id:'manta', publicId:'jugg_manta', name:'Manta Style', priority:81, trigger:[['item_owned',getItem(ITEMS,'manta').id]], expectedMinute:19, earlyToleranceMin:2.2, lateToleranceMin:3.8, activeDurationSec:300, fadeDurationSec:220, permanent:{farm:13,push:16,survival:13}, window:{pressure:16,fight:12}, actions:{PRESSURE:17,CONNECT:11,FIGHT:10}, recommendation:'Pressure a lane with dispel available and connect only when Omnislash has a credible isolated target.' }),
