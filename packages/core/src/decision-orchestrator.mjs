...
const CONFLICT_PAIRS = [
  ['FARM', 'FIGHT'], ['FARM', 'OBJECTIVE'], ['RESET', 'FIGHT'], ['RESET', 'OBJECTIVE'],
  ['HOLD_LANE', 'ROTATE'], ['PROTECT_CORE', 'MOVE_TO_WISDOM'], ['PROTECT_CARRY', 'MOVE_TO_WISDOM'],
  ['TAKE_ROSHAN', 'DEFEND_TOWER'], ['PRESSURE', 'CONNECT'], ['BUILD', 'URGENT_GAMEPLAY'],
  ['STACK', 'PROTECT_CARRY'], ['PULL', 'PROTECT_CARRY']
];
...
export function actionsConflict(a, b) {
  ...
  return CONFLICT_PAIRS.some(([p, q]) => (x === p && y === q) || (x === q && y === p)) ||
    ((a?.startsWith('BUY_') || b?.startsWith('BUY_')) && [a, b].some(x => ['CRITICAL', 'RESET'].includes(x)));
}
...
function urgencyFor(action, domain, decision) {
  ...
  if (action === 'RESET' || action === 'RESET_BEFORE_OBJECTIVE' || action === 'PROTECT_CORE' || action === 'PROTECT_CARRY') return 'CRITICAL';
...
export function orchestrateDecision(input = {}) {
  ...
  const candidates = [];
  const hp = hpOf(state); const alive = state.alive !== false;
  ...
  const winner = candidates[0]; const suppressed = candidates.slice(1).map(c => ({ action: c.action, domain: c.domain, confidence: c.confidence, score: +c.score.toFixed(3), reason: actionsConflict(winner.action, c.action) ? 'Conflicts with selected call' : 'Lower priority or longer window' }));
  ...
  return {
    primaryAction: winner.action, primaryDomain: winner.domain, title: winner.action.replaceAll('_', ' '),
    instruction: winner.raw?.message ?? winner.raw?.instruction ?? winner.reasons[0] ?? 'Maintain the selected safe plan.',
    confidence: winner.confidence, urgency: winner.urgency, reasons: winner.reasons.slice(0, 6),
    steps: [...(winner.raw?.steps ?? [])].slice(0, 5), secondaryActions, suppressedActions: suppressed,
    conflicts, blockers: winner.blockers, missingSignals,
    cancellationConditions: ['NEW_MATCH', 'HERO_DIED', 'TTL_EXPIRED', ...(winner.domain === 'OBJECTIVE' ? ['OBJECTIVE_WINDOW_CLOSED', 'HP_BELOW_SAFE_THRESHOLD', 'KEY_COOLDOWN_UNAVAILABLE'] : []), ...(winner.domain === 'BUILD' ? ['TARGET_ITEM_PURCHASED'] : [])],
    expiresAtSec: now + ttl, nextEvaluationSec: now + (winner.urgency === 'CRITICAL' ? 1 : 5),
    dataQuality: winner.quality, strategyTrace: {
      considered: candidates.map(c => ({ action: c.action, domain: c.domain, confidence: c.confidence })),
      selected: { action: winner.action, domain: winner.domain, confidence: winner.confidence },
      suppressed: suppressed.map(({ action, domain, reason }) => ({ action, domain, reason })),
      selectionReasons: [...winner.reasons, winner.windowSec <= 45 ? 'This opportunity has the shortest confirmed window' : 'Highest safe calibrated priority'].slice(0, 6),
      safetyRules: candidates.filter(c => c.domain === 'SAFETY').map(c => c.action), missingSignals
    }, generatedAtSec: now
  };
}
...
export class DecisionOrchestratorCoordinator {
  ...
  update(input, reason = 'EVENT') {
    ...
    const cancelled = current && (now >= current.expiresAtSec || input.state?.alive === false && current.primaryAction !== 'WAIT_RESPAWN');
    ...
    if (meaningfulCoachCallChange(current, proposed)) { this.history.push({ gameTimeSec: now, previousAction: current?.primaryAction ?? null, action: proposed.primaryAction, urgency: proposed.urgency, confidence: proposed.confidence, reason: emergency ? 'EMERGENCY_OVERRIDE' : cancelled ? 'CANCELLATION_OR_EXPIRY' : reason, dataQuality: proposed.dataQuality }); if (this.history.length > this.options.historyLimit) this.history.splice(0, this.history.length - this.options.historyLimit); }
    this.current = proposed; return proposed;
  }
}

export function formatCoachCall(call, mode = 'STANDARD') {
  if (!call) return null; const pct = Math.round(call.confidence * 100), next = call.secondaryActions?.[0]?.action?.replaceAll('_', ' ');
