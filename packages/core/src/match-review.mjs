...
export function buildDecisionTimeline(snapshot, extras = []) {
  ...
  snapshot.coachCallHistory?.forEach((x,i)=>rows.push(entry(x,i,'COACH_CALL',snapshot.state)));
...
export function calculateDecisionMetrics(timeline, outcomes = []) {
  ...
  const calls=timeline.filter(x=>x.category==='COACH_CALL'), callOutcomes=outcomes.filter(x=>calls.some(c=>c.id===x.decisionId));
  return { decisionSuccessRate:rate(resolved.filter(x=>['SUCCESS','PARTIAL_SUCCESS'].includes(x.outcome)).length,resolved.length), falseFightRate:rate(failures('FIGHT'),macro.filter(x=>x.action==='FIGHT').length), lateConnectRate:null, unnecessaryResetRate:rate(failures('RESET'),macro.filter(x=>x.action==='RESET').length), decisionStability:Math.max(0,Math.round((1-Math.min(1,macro.length/(duration/30)))*1000)/10), decisionChurn:Math.round(macro.length/(duration/60)*10)/10, powerSpikeConversion:null, roleTaskCompletion:null, objectiveConversion:null, survivalAfterWarning:null, economyGrowthAfterFarm:null, pressureValue:null, dataQualityScore:Math.round(quality*1000)/10, coachCallStability:Math.max(0,100-calls.length*2), criticalCallSurvivalRate:rate(callOutcomes.filter(x=>x.action?.includes('RESET')&&x.outcome!=='FAILURE').length,callOutcomes.filter(x=>x.action?.includes('RESET')).length), safeFallbackRate:rate(calls.filter(x=>/PREPARE|HOLD_SAFE/.test(x.action)).length,calls.length), objectiveCallConversion:rate(callOutcomes.filter(x=>/ROSHAN|TORMENTOR|TOWER/.test(x.action)&&x.outcome==='SUCCESS').length,callOutcomes.filter(x=>/ROSHAN|TORMENTOR|TOWER/.test(x.action)).length), callResolutionRate:rate(callOutcomes.filter(x=>!['UNRESOLVED','INSUFFICIENT_DATA'].includes(x.outcome)).length,callOutcomes.length), contradictoryCallRate:0 };
}

export function createMatchReview(snapshot, { states = [], extras = [], outcome = null } = {}) {
  const timeline = buildDecisionTimeline(snapshot,extras); const important = timeline.filter(x=>['MACRO','ROLE','LANE','OBJECTIVE','COACH_CALL'].includes(x.category));
